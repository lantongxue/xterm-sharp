...
const scenarioPath = process.argv[2] ?? 'tools/marker-scenarios.json';
...
for (const scenario of scenarios) {
  if (!scenario.name || !scenario.request || names.has(scenario.name)) {
    console.error(`Invalid or duplicate marker scenario '${scenario.name ?? ''}'.`);
    process.exit(2);
  }
...
console.log(`MATCH ${matched}/${scenarios.length} marker and metadata scenarios`);
