...
{
    private sealed record Protocol(
        TerminalMouseEventTypes Events,
        Func<TerminalMouseEvent, (bool Allowed, TerminalMouseEvent Event)> Restrict);

    private readonly Dictionary<string, Protocol> _protocols = new(StringComparer.Ordinal)
    {
    ...
    public string ActiveProtocol
    {
        ...
        set
        {
            if (!_protocols.TryGetValue(value, out Protocol? protocol))
            {
        ...
        Func<TerminalMouseEvent, (bool Allowed, TerminalMouseEvent Event)> restrict)
    {
        ...
        _protocols[name] = new Protocol(events, restrict);
    }
